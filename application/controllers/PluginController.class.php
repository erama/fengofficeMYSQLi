...
static function executeInstaller($name) {
	...
	try {
		...
		if (file_exists ( $path )) {
			...
			$schema_creation = ROOT . "/plugins/$name/install/sql/mysqli_schema.php";
			...
			// Create schema sql query
			$schema_query = ROOT . "/plugins/$name/install/sql/mysqli_initial_data.php";
			if (file_exists ( $schema_query )) {
				$total_queries = 0;
				$executed_queries = 0;
				executeMultipleQueries ( tpl_fetch ( $schema_query ), $total_queries, $executed_queries );
				Logger::log ( "Initial data loaded for plugin  '$name'." . mysqli_error () );
			}
			...
			if (file_exists ( $install_script )) {
				...
				Logger::log ( "File install.php processed for plugin  '$name'." . mysqli_error () );
			}
