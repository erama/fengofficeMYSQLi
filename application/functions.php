...
function feng__autoload($load_class_name) {
	...
	if(!$loader) {
		...
		$res = mysqli_query("SELECT name FROM ".TABLE_PREFIX."plugins WHERE is_installed = 1;");
...
function get_table_columns($table_name) {
	$cols = array();
	$res = mysqli_query("DESCRIBE `$table_name`", DB::connection()->getLink());
...
function checkTableExists($table_name) {
	$res = mysqli_query("SHOW TABLES", DB::connection()->getLink());
	while ($row = mysqli_fetch_array($res)) {
