...
//				echo "<br><pre>".print_r($font)."</pre><br>";
		...
		public function delete($recursive = false){
			...
			//PEPE
			if (
				@mysqli_query("DELETE FROM `" . table('cells') . "` WHERE `SheetId` IN (SELECT `SheetId` FROM `" . table('sheets') . "` WHERE `BookId` = $bookId)") &&
				@mysqli_query("DELETE FROM `" . table('mergedCells') . "` WHERE `SheetId` IN (SELECT `SheetId` FROM `" . table('sheets') . "` WHERE `BookId` = $bookId)") &&
				@mysqli_query("DELETE FROM `" . table('rows') . "` WHERE `SheetId` IN (SELECT `SheetId` FROM `" . table('sheets') . "` WHERE `BookId` = $bookId)") &&
				@mysqli_query("DELETE FROM `" . table('columns') . "` WHERE `SheetId` IN (SELECT `SheetId` FROM `" . table('sheets') . "` WHERE `BookId` = $bookId)") &&
				@mysqli_query("DELETE FROM `" . table('sheets') . "` WHERE `BookId` = $bookId") &&
				@mysqli_query("DELETE FROM `" . table('fontStyles') . "` WHERE `BookId` = $bookId") &&
				@mysqli_query("DELETE FROM `" . table('books') . "` WHERE `BookId` = $bookId" ))
			{
				//throw new Success('Book deleted succesfully!',"{'BookId':$bookId}");
			} else {
				$error = new GsError(302,"Error deleting book.");
				if($error->isDebugging()){
					$err = str_replace("'", '"', mysqli_error());
		...
		public function save(){
			...
			if (isset($this->bookId)) { // Edit book..
				...
				$res = mysqli_query("SELECT BookId FROM ".table('books'). " where BookId=$this->bookId");
				if(!$res){
					$error = new GsError(302,"Error loading book.");
					if($error->isDebugging()){
						$error->addContentElement("BookId",$BookId);
