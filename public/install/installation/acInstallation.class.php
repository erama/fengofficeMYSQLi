...
final class acInstallation {
	...
	function execute() {
		...
		if($this->database_connection = @mysqli_connect($database_host, $database_user, $database_pass)) {
			$connected = @mysqli_select_db($database_name, $this->database_connection);
		} // if
		...
		@mysqli_query('BEGIN WORK', $this->database_connection);

		// Database construction
		$total_queries = 0;
		$executed_queries = 0;
		if($this->executeMultipleQueries(tpl_fetch(get_template_path('sql/mysqli_schema.php')), $total_queries, $executed_queries)) {
		...
		$total_queries = 0;
		$executed_queries = 0;
		if($this->executeMultipleQueries(tpl_fetch(get_template_path('sql/mysqli_initial_data.php')), $total_queries, $executed_queries)) {
		...
		@mysqli_query('COMMIT', $this->database_connection);
	...
	function breakExecution($error_message) {
		...
		if(is_resource($this->database_connection)) {
			@mysqli_query('ROLLBACK', $this->database_connection);
		} // if
		return false;
	} // breakExecution
	...
	function haveInnoDbSupport() {
		...
		if($mysqli_version && (version_compare($mysqli_version, '5.6', '>=') || strpos($mysqli_version, 'MariaDB') !== false)){
			//mysql 5.6 have_innodb is deprecated
			if($res = mysqli_query("SHOW ENGINES", $this->database_connection)){
			...
			if($result = mysqli_query("SHOW VARIABLES LIKE 'have_innodb'", $this->database_connection)) {
		...
		if (count($pluginNames)) {
			foreach  ($pluginNames as $name ) {
				...
				if (file_exists($path)) {
					...
					if (count(array_var($pluginInfo,'types'))){
						foreach ($pluginInfo['types'] as $k => $type ) {
							if (isset($type['name'])) {
								...
								if (@mysqli_query($sql)){
									$pluginInfo['types'][$k]['id'] = @mysqli_insert_id() ;
									$type['id'] =  @mysqli_insert_id() ;
					...
					$schema_creation = INSTALLATION_PATH."/plugins/$name/install/sql/mysqli_schema.php" ;
					if ( file_exists($schema_creation) ){
						...
						if($this->executeMultipleQueries(tpl_fetch($schema_creation), $total_queries, $executed_queries)) {
							...
							$this->breakExecution("Error while creating schema for plugin $name".mysqli_error());
							//$this->printMessage("Error while creating schema for plugin $name".mysqli_error());
						}
					} 

					// Create schema sql query
					$schema_query = INSTALLATION_PATH."/plugins/$name/install/sql/mysqli_initial_data.php" ;
					if ( file_exists($schema_query) ){
						$total_queries = 0;
						$executed_queries = 0;
						if($this->executeMultipleQueries(tpl_fetch($schema_query), $total_queries, $executed_queries)) {
							$this->printMessage("Initial data loaded for plugin  '$name'.".mysqli_error());	
						}else{
							$this->breakExecution("Error while loading inital data for plugin '$name'.".mysqli_error());
						}	
					}
					...
					if ( file_exists($install_script) ){
						...
						if($this->executeMultipleQueries(implode("\n", $queries), $total_queries, $executed_queries)) {
							...
							echo mysqli_error();
							$this->breakExecution("Error while executing install.php for plugin '$name'.".mysqli_error());
							DB::rollback();
							return false;
						}
					}
