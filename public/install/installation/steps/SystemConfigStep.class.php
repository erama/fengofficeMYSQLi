...
class SystemConfigStep extends ScriptInstallerStep {
			...
			if ($this->database_connection) {
				...
				if (!$connected) {
					...
					if ($errno == 1049 && strpos($database_name, '`') === false) {
						...
						if (@mysqli_query('CREATE DATABASE `' . $database_name . '`', $this->database_connection)) {
							$connected = @mysqli_select_db($database_name, $this->database_connection);
						}
					}
				}
	...
	function breakExecution($error_message) {
		...
		if(is_resource($this->database_connection)) @mysqli_query('ROLLBACK', $this->database_connection);
