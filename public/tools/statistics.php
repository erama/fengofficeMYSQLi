...
if (!mysqli_select_db(DB_NAME, $db_link)) die();
...
if (isset($_REQUEST['modules']) && $_REQUEST['modules']) {
	...
	$db_res = mysqli_query("select count(*) as num, o.object_type_id, ot.name from ".TABLE_PREFIX."objects o inner join ".TABLE_PREFIX."object_types ot on o.object_type_id=ot.id where trashed_by_id=0 group by o.object_type_id order by ot.name;", $db_link);
	...
	$db_res = mysqli_query("select count(*) as num from ".TABLE_PREFIX."contacts c where c.user_type > 0");
	while ($row = mysqli_fetch_assoc($db_res)) {
		$info['Users'] = array(
			'objects' => $row['num'],
			'log_info' => array('total' => 0, 'details' => array()),
	...
	// members
	$members = array();
	$db_res = mysqli_query("select count(*) as num, ot.name as type, p.name as plugin from ".TABLE_PREFIX."members m 
...
if (isset($_REQUEST['logins']) && $_REQUEST['logins']) {
	...
	$db_res = mysqli_query("SELECT `created_on`, `object_name`, `rel_object_id` FROM `".TABLE_PREFIX."application_logs` WHERE `action` = 'login' AND `created_on` > ADDDATE(NOW(), INTERVAL -1 MONTH) ORDER BY `rel_object_id`, `created_on` desc", $db_link);
	while ($row = mysqli_fetch_assoc($db_res)) {
		$last_logins[] = array(
...
if (isset($_REQUEST['activity']) && $_REQUEST['activity']) {
	...
	$db_res = mysqli_query("select o.id, o.name, pg.name as `type`, ce.email_address as email, c.last_activity as `date` from ".TABLE_PREFIX."contacts c 
	...
	$db_res = mysqli_query("select count(*) as num, DATE(created_on) as created from ".TABLE_PREFIX."application_logs WHERE created_by_id>0 AND action in ('add','edit','trash','comment') AND created_on > ADDDATE(NOW(), INTERVAL -1 MONTH) group by created;");
	while ($row = mysqli_fetch_assoc($db_res)) {
...
if (isset($_REQUEST['sizes']) && $_REQUEST['sizes']) {
	...
	$db_res = mysqli_query("select sum(data_length + index_length) as bytes from `information_schema`.`TABLES` WHERE table_schema = '".DB_NAME."'");
	while ($row = mysqli_fetch_assoc($db_res)) {
		$sizes['db'] = $row['bytes'];
	}
...
if (isset($_REQUEST['plugins']) && $_REQUEST['plugins']) {
	...
	$db_res = mysqli_query("SELECT id,name,version FROM ".TABLE_PREFIX."plugins WHERE is_installed=1 AND is_activated=1");
	while ($row = mysqli_fetch_assoc($db_res)) {
		$plugins[] = $row;
	}
