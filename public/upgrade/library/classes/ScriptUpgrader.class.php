...
final class ScriptUpgrader {
	...
	function upgrade($version_from, $version_to) {
		...
		if (is_array($scripts)) {
			...
			// check MySQL version
			$mysqli_version = mysqli_get_server_info($dbc);
			...
			if (isset($last_correct_version)) {
				@mysqli_query("UPDATE `".TABLE_PREFIX."config_options` SET `value` = 0 WHERE `name` = 'upgrade_last_check_new_version'");
				tpl_assign('version', $last_correct_version);
