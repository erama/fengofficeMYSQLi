...
class AsadoUpgradeScript extends ScriptUpgraderScript {
	...
	function execute() {
		...
		if (version_compare($installed_version, '2.0.0.0-beta') < 0) {
			...
			if (!defined('FILE_STORAGE_MYSQL')) define('FILE_STORAGE_MYSQL', 'mysqli');
			...
			if (version_compare($installed_version, '2.0.0.6') <= 0) {
				...
				if (@mysqli_fetch_row(@mysqli_query(("SELECT id from ".$t_prefix."plugins WHERE name = 'workspaces'")))) {
				...
				if ($obj = @mysqli_fetch_object(@mysqli_query("SELECT id FROM ".$t_prefix."object_types WHERE name = 'workspace' " ))) {
					$wsTypeId = $obj->id ;
					$res = @mysqli_query("SELECT * FROM ".$t_prefix."members WHERE dimension_id = (SELECT id FROM ".$t_prefix."dimensions WHERE code='workspaces')" ) ; 
					while ( $m = @mysqli_fetch_object($res) ) {
						@mysqli_query("INSERT INTO ".$t_prefix."objects (object_type_id, name) VALUES ($wsTypeId, '".$m->name."' )" );
						if ( $id = @mysqli_insert_id()){
							@mysqli_query("INSERT INTO ".$t_prefix."workspaces (object_id) VALUES ($id)");
							@mysqli_query("UPDATE ".$t_prefix."members SET object_id=$id WHERE id = $m->id ");
						}
					}
				}
			}
